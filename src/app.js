...
class App {
    ...
    try {
      ...
      if (savedConfig) {
        ...
        if (parsed.api && !parsed.api.modules) {
          Logger.info('[App] Migrating old config — adding modules...');
          parsed.api.modules = config.get().api.modules;
  ...
  _appendLog(container, ts, cls, msg) { const line = document.createElement('div'); line.className = `log-line ${cls}`; const span = document.createElement('span'); span.className = 'timestamp'; span.textContent = `[${ts}]`; line.appendChild(span); line.append(` ${msg}`); container.appendChild(line); container.scrollTop = container.scrollHeight; }
  allocationLog(msg, type = 'info') { if (!this.elements.allocationTerminal) return; const ts = new Date().toLocaleTimeString('id-ID'); const cls = type === 'error' ? 'log-error' : type === 'success' ? 'log-success' : 'log-info'; this._appendLog(this.elements.allocationTerminal, ts, cls, msg); }

  log(msg, type = 'info') { if (!this.elements.logTerminal) return; const ts = new Date().toLocaleTimeString('id-ID'); const cls = type === 'error' ? 'log-error' : type === 'success' ? 'log-success' : type === 'warning' ? 'log-warning' : type === 'request' ? 'log-request' : 'log-info'; this._appendLog(this.elements.logTerminal, ts, cls, msg); }
